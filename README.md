...
1. Файл **app.py** представляет собой основное Flask приложение
2. Файл **models.py** содержит SQLAlchemy для модели БД
3. Файл **config.py** содержит конфигурацию приложения
4. Файл **requirements.txt**  описывает Python-зависимости (тербуемые версии библиотек)
5. Файл **Dockerfile**  предназначен для генерации Docker-образ для Flask приложения
6. Файл **docker-compose.yml** обеспечивает Оркестрацию двух контейнеров: python-приложения и СУБД PostgreSQL
7. Файл **.env** содержит переменные окружения
8. Каталог **uploads/** представляет собой подключаемый Volume для хранения файлов изображений (смонтируется в контейнер)
9. Файл **init_db.sql**  предназначен для инициализация СУБД PostgreSQL
